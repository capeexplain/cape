...
class TableCanvas(Canvas):
    ...
    def get_currentColName(self):
        ...
        colname = self.model.getRecName(self.currentcol)
    ...
    def get_currentRecCol(self):
        ...
        print('recname: '+recname+' colname: '+colname)
    ...
    def get_col_clicked(self,event):
        ...
        for colpos in self.col_positions:
            ...
            if x > colpos and x <= nextpos:
                ...
                print(self.col_positions.index(colpos))
            else:
                #print None
                pass
        #return colc
    ...
    def handle_left_click(self, event):
        ...
        self.clearSelected()
        self.allrows = False
        self.rowclicked = self.get_row_clicked(event)
        self.colclicked = self.get_col_clicked(event)
        self.focus_set()
        if self.mode == 'formula':
            self.handleFormulaClick(self.rowclicked, self.colclicked)
            return
        ...
        self.startrow = self.rowclicked
        self.endrow = self.rowclicked
        self.startcol = self.colclicked
        self.endcol = self.colclicked
        #reset multiple selection list
        self.multiplerowlist=[]
        self.multiplerowlist.append(self.rowclicked)
        if 0 <= self.rowclicked < self.rows and 0 <= self.colclicked < self.cols:
            self.setSelectedRow(self.rowclicked)
            self.setSelectedCol(self.colclicked)
            self.drawSelectedRect(self.currentrow, self.currentcol)
            self.drawSelectedRow()
            self.tablerowheader.drawSelectedRows(self.rowclicked)
            coltype = self.model.getColumnType(self.colclicked)
            if coltype == 'text' or coltype == 'number':
                self.drawCellEntry(self.rowclicked, self.colclicked)
        return
    ...
    def handle_left_ctrl_click(self, event):
        ...
        self.colclicked = self.get_col_clicked(event)
        if 0 <= self.rowclicked < self.rows and 0 <= self.colclicked < self.cols:
            if self.rowclicked not in self.multiplerowlist:
                self.multiplerowlist.append(self.rowclicked)
            else:
                self.multiplerowlist.remove(self.rowclicked)
            self.drawMultipleRows(self.multiplerowlist)
            if self.colclicked not in self.multiplecollist:
                self.multiplecollist.append(self.colclicked)
    ...
    def handle_right_click(self, event):
        ...
        self.rowclicked = self.get_row_clicked(event)
        self.colclicked = self.get_col_clicked(event)
        if self.colclicked == None:
            self.rightmenu = self.popupMenu(event, outside=1)
            return

        if (self.rowclicked in self.multiplerowlist or self.allrows == True) and self.colclicked in self.multiplecollist:
...
class ColumnHeader(Canvas):
    ...
    def handle_left_click(self,event):
        ...
        self.colclicked = self.table.get_col_clicked(event)
        if self.colclicked == None:
            return
        #set all rows selected
        self.table.allrows = True
        self.table.setSelectedCol(self.colclicked)
    ...
    def handle_left_shift_click(self, event):
        ...
        self.colclicked = self.table.get_col_clicked(event)
        if self.colclicked > currcol:
            self.table.multiplecollist = range(currcol, self.colclicked+1)
        elif self.colclicked < currcol:
            self.table.multiplecollist = range(self.colclicked, currcol+1)
        else:
            return
...
class RowHeader(Canvas):
    ...
    def handle_left_click(self, event):
        self.rowclicked = self.table.get_row_clicked(event)
        self.startrow = self.rowclicked
        if 0 <= self.rowclicked < self.table.rows:
            self.delete('rect')
            self.table.delete('entry')
            self.table.delete('multicellrect')
            #set row selected
            self.table.setSelectedRow(self.rowclicked)
    ...
    def handle_left_ctrl_click(self, event):
        ...
        if 0 <= self.rowclicked < self.table.rows:
            if self.rowclicked not in multirowlist:
                multirowlist.append(self.rowclicked)
            else:
                multirowlist.remove(self.rowclicked)
