...
class pylabPlotter(object):
    ...
    def plotSetup(self, data=None):
        ...
        row=0
        scalesframe = LabelFrame(self.plotprefswin, text="Axes tk.Scales")
        scales={0:'norm',1:'log'}
        for i in range(0,2):
        ...
        tk.Entry(labelsframe,textvariable=self.plottitlevar,bg='white',relief=GROOVE).grid(row=0,column=1,padx=2,pady=2)
        Label(labelsframe,text='X-axis label:').grid(row=1,column=0,padx=2,pady=2)
        tk.Entry(labelsframe,textvariable=self.plotxlabelvar,bg='white',relief=GROOVE).grid(row=1,column=1,padx=2,pady=2)
        Label(labelsframe,text='Y-axis label:').grid(row=2,column=0,padx=2,pady=2)
        tk.Entry(labelsframe,textvariable=self.plotylabelvar,bg='white',relief=GROOVE).grid(row=2,column=1,padx=2,pady=2)
        ...
        if self.currdata != None:
            ...
            for d in range(len(self.dataseriesvars)):
                ...
                cbuttons[d]=tk.Button(frame,text='Series '+str(sr),bg=c,command=action)
        ...
        frame.grid(row=row,column=0,columnspan=2,sticky='news',padx=2,pady=2)
        replotb = tk.Button(frame, text="Replot",
        ...
        b = tk.Button(frame, text="Apply", command=self.applyOptions, relief=GROOVE, bg='#99ccff')
        b.pack(side=LEFT,fill=X,padx=2,pady=2)
        b = tk.Button(frame, text="Save", command=self.saveCurrent, relief=GROOVE, bg='#99ccff')
        b.pack(side=LEFT,fill=X,padx=2,pady=2)
        c=tk.Button(frame,text='Close', command=close_prefsdialog, relief=GROOVE, bg='#99ccff')
