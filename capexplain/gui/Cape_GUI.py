...
class CAPE_UI:
        """
        CAPE_UI is a GUI frontend for cape used to run queries and explore explanations generated by Cape.
        """
    def __init__(self, parent, conn, config, pattern_schema='dev', assigned_local_table='_local', assigned_global_table='_global'):
        ...
        logger.debug('self.assigned_local_table is:' +
                     str(self.assigned_local_table))
        logger.debug('self.assigned_global_table is:' +
                     str(self.assigned_global_table))
...
#----------------------------------place frames----------------------------------#
        ...
        self.main_frame.grid(column=0, row=0, columnspan=3,
                             rowspan=10, sticky='nsew')

        self.table_frame = ttk.Frame(
            self.main_frame, borderwidth=5, relief="sunken")
        self.table_frame.grid(column=0, row=0, columnspan=1,
                              rowspan=10, sticky='nsew')

        self.query_frame = ttk.Frame(
            self.main_frame, borderwidth=5, relief="ridge")
        self.query_frame.grid(column=1, row=0, columnspan=1,
        ...
        self.query_result = ttk.Frame(
            self.main_frame, borderwidth=5, relief="sunken")
        self.query_result.grid(
            column=1, row=2, columnspan=1, rowspan=8, sticky='nsew')

        self.local_pattern = ttk.Frame(
            self.main_frame, borderwidth=5, relief="sunken")
        self.local_pattern.grid(
            column=2, row=0, columnspan=2, rowspan=5, sticky='nsew')

        self.explanation_frame = ttk.Frame(
            self.main_frame, borderwidth=5, relief="sunken")
        self.explanation_frame.grid(
            column=2, row=5, columnspan=2, rowspan=5, sticky='nsew')


#---------------------------table frame-----------------------------------------#
        self.table_view = ttk.Treeview(self.table_frame, height=47)
        self.table_info = ttk.Label(self.table_frame, text="Database Information", font=(
            'Times New Roman bold', 11), borderwidth=5, relief=RIDGE)
        ...
        for key, value in self.db_info_dict.items():
            self.table_view.insert(
                '', 'end', 'item'+str(table_index), text=key)
            for n in value:
...
#----------------------------Query frame----------------------------------------#
        ...
        self.query_temp = User_Query_Frame(
            conn=self.conn, table_dict=self.db_info_dict, parent=self.query_frame)

        self.query_button = Button(self.query_button_frame, text="Run Query", font=(
            'Times New Roman bold', 12), command=self.run_query)
        self.query_button.pack(side=RIGHT)
        self.show_global_pattern_button = Button(self.query_button_frame, text="Show Global Pattern", font=(
            'Times New Roman bold', 12), command=self.show_global_pattern)
        self.show_global_pattern_button.pack(side=RIGHT)
        self.show_local_pattern_button = Button(self.query_button_frame, text="Show Local Pattern", font=(
            'Times New Roman bold', 12), command=self.show_local_pattern)
...
#----------------------------Query result frame --------------------------------#
        ...
        self.result_label = Label(self.query_result, text='Query Result', font=(
            'Times New Roman bold', 12), borderwidth=5, relief=RIDGE)
        ...
        self.high_button = Button(self.high_low_frame, text='High', font=(
            'Times New Roman bold', 12), command=self.handle_high)
        self.high_button.grid(column=0, row=1)

        self.low_button = Button(self.high_low_frame, text='Low', font=(
            'Times New Roman bold', 12), command=self.handle_low)
...
#---------------------------Global Pattern Frame -----------------------------------#
        ...
        self.global_pattern_label = Label(self.show_global, text="Global Patterns", font=(
            'Times New Roman bold', 12), borderwidth=5, relief=RIDGE)
        ...
        self.global_description_button = Button(self.high_low_frame, text='Describe\nGlobal', font=(
            'Times New Roman bold', 12), command=self.global_description)
        ...
        self.global_pattern_filter_button = Button(self.high_low_frame, text='Filter \nLocal\n Pattern', font=(
            'Times New Roman bold', 12), command=self.use_global_filter_local)
...
#------------------------------- local pattern frame---------------------------------------#
        ...
        self.local_pattern_label = Label(self.local_pattern, text="Local Patterns", font=(
            'Times New Roman bold', 12), borderwidth=5, relief=RIDGE)
        self.local_pattern_label.grid(
            column=0, row=0, columnspan=5, sticky='nsew')
        self.local_pattern_filter_button = Button(self.local_pattern, text='Reset Query Output', font=(
            'Times New Roman bold', 12), command=self.reset_output)
        self.local_pattern_filter_button.grid(column=0, row=2)
        self.local_pattern_filter_button = Button(self.local_pattern, text='Filter Output', font=(
            'Times New Roman bold', 12), command=self.show_updated_output)
        self.local_pattern_filter_button.grid(column=2, row=2)
        self.draw_pattern_button = Button(self.local_pattern, text='Draw Pattern', font=(
            'Times New Roman bold', 12), command=self.pop_up_pattern)
        self.draw_pattern_button.grid(column=4, row=2)

        self.local_pattern_table_frame = Frame(self.local_pattern)
        self.local_pattern_table_frame.grid(
...
#---------------------------------explanation frame-----------------------------#
        ...
        self.exp_label = Label(self.explanation_frame, text="Top Explanations", font=(
            'Times New Roman bold', 12), borderwidth=5, relief=RIDGE)
        ...
        self.exp_table.show()
        self.describe_exp_button = Button(self.explanation_frame, text="Describe Explanation", font=(
            'Times New Roman bold', 12), command=self.pop_up_explanation)
...
#----------------------------------Functions----------------------------------------#
    ...
    def show_global_pattern(self):
        ...
        global_query = "select array_to_string(fixed,',') as Partition,array_to_string(variable,',') as Predictor,agg," +\
            ...
            " where array_to_string(array_sort(fixed||variable),',')='" + \
            self.query_group_str+"';"
    ...
    def show_local_pattern(self):
        ...
        for n in self.local_pattern_table.multiplerowlist:
            self.chosen_local_pattern = self.local_output_pattern_df.iloc[int(
                n)]
        ...
        self.local_output_pattern_df['stats'] = self.local_output_pattern_df['stats'].str.split(
            ',', expand=True)[0]
        self.local_output_pattern_df['stats'] = self.local_output_pattern_df['stats'].str.strip(
            '[')
        self.local_output_pattern_df["stats"] = pd.to_numeric(
            self.local_output_pattern_df["stats"])
        self.local_output_pattern_df["stats"] = self.local_output_pattern_df["stats"].round(
            2)
    ...
    def use_global_filter_local(self):
        ...
        for n in self.global_pattern_table.multiplerowlist:
            ...
            self.local_output_pattern_df = pd.read_sql(
                g_filter_l_query, self.conn)
            self.local_output_pattern_df['stats'] = self.local_output_pattern_df['stats'].str.split(
                ',', expand=True)[0]
            self.local_output_pattern_df['stats'] = self.local_output_pattern_df['stats'].str.strip(
                '[')
    ...
    def global_description(self):
        ...
        for n in self.global_pattern_table.multiplerowlist:
            ...
            variable_attribute = self.global_pattern_df.iloc[int(
                n)]['predictor']
        ...
        global_desc = "For each ("+fixed_attribute+'), the '+aggregation_function + ' is '+modeltype+'\n in '+variable_attribute+'.' +\
            ...
            str(Lambda*100) + ' % of the '+fixed_attribute
        desc_win = Toplevel()
        ...
        desc_label = Label(desc_frame, text=global_desc, font=(
            'Times New Roman bold', 12), borderwidth=5, relief=SOLID, justify=LEFT)
    ...
    def use_local_filter_output(self):
        ...
        for n in self.local_pattern_table.multiplerowlist:
            ...
            partition_attr_list = self.local_output_pattern_df.iloc[int(
                n)]['partition'].split(',')
            partition_value_list = self.local_output_pattern_df.iloc[int(
        ...
        for n in range(len(partition_attr_list)):
            if(self.query_data_convert_dict[partition_attr_list[n]] == 'str'):
                condition = "{} =\'{}\'".format(
                    partition_attr_list[n], partition_value_list[n])
            elif(self.query_data_convert_dict[partition_attr_list[n]] == 'float'):
                condition = "{} = {}::float".format(
                    partition_attr_list[n], partition_value_list[n])
            else:
                condition = "{} = {}::int".format(
                    partition_attr_list[n], partition_value_list[n])
    ...
    def handle_question(self, direction):
        ...
        eg = ExplanationGenerator(
            config, {'pattern_table': self.config.pattern_table})
        eg.initialize()
        col_name = ['Explanation_Tuple', "Score", 'From_Pattern', "Drill_Down_To", "Distance", "Outlierness",
                    "Denominator", "relevent_model", "relevent_param", "refinement_model", "drill_param"]
        exp_df = pd.DataFrame(columns=["From_Pattern", "Drill_Down_To", "Score", "Distance", "Outlierness",
                                       "Denominator", "relevent_model", "relevent_param", "refinement_model", "drill_param"])
        for n in self.query_result_table.multiplerowlist:
            ...
            self.question.rename(
                columns={self.agg_name: self.user_agg}, inplace=True)
            ...
            for e in elist:
                ...
                if e.expl_type == 1:
                    local_pattern = (
                        ...
                        ','.join(
                            list(map(str, e.relevent_pattern[2])))+' \u2933 '+self.agg_name
                    )
                    relevent_model = e.relevent_pattern[4]
                    if e.relevent_pattern[4] == 'const':
                        relevent_param = str(
                            round(float(e.relevent_pattern[6].split(',')[0][1:]), 2))
                    else:
                    ...
                    drill_down_to = ','.join(
                        [x for x in e.refinement_pattern[0] if x not in e.relevent_pattern[0]])
                    refinement_model = e.refinement_pattern[4]
                    if e.refinement_pattern[4] == 'const':

                        drill_param = str(
                            round(float(e.refinement_pattern[6].split(',')[0][1:]), 2))
                    else:
    ...
    def get_pattern_result(self, partition_attr_list=None, partition_value_list=None, pred_attr_list=None):
    ...
    def pop_up_explanation(self):
        ...
        for n in self.exp_table.multiplerowlist:
            ...
            rel_pattern_part_value = relevent_pattern.split(':')[0].split('=')[
                1].split(']')
            rel_pattern_pred = relevent_pattern.split(
                ':')[1].split(' \u2933 ')[0]
            ...
            if(drill_attr != ''):
                exp_tuple_col = rel_pattern_part_list + \
                    rel_pattern_pred_list + [drill_attr]
                exp_tuple_col.sort()
            else:
...
def startCapeGUI(conn, config, local_table, global_table):
        ...
        local_table : str
             Database table storing local patterns.
        global_table : str
             Database table storing global patterns.
        """
    ...
    ui = CAPE_UI(root, conn=conn, config=config,
                 assigned_local_table=local_table, assigned_global_table=global_table)
    root.mainloop()

def main():
        """
        The main function opens the GUI using default database connection parameters
        """

    config = DictLike()
    dbconn = DBConnection(host="127.0.0.1", user="antiprov",
                          db="antiprov", password='1234', port="5432")
    local_table = dbconn['local_table']
    global_table = dbconn['global_table']
    conn = dbconn.pgconnect()
    startCapeGUI(conn=conn, config=config, assigned_local_table=local_table,
                 assigned_global_table=global_table)
